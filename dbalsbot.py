...
* 강타는 90초에 한 번씩 충전되며, 최대 2회까지 충전됩니다. 해당 주문을 한 번 사용한 후, 재사용 대기시간 15초가 적용됩니다.""")
    ...
    if message.content.startswith("/순간이동") or message.content.startswith("/텔") or message.content.startswith("/텔레포트"):
    ...
    if message.content.startswith("/점화"):
        embed = discord.Embed(title="드발스봇", description="점화", color=0x1ca54d)
...
client.run("ODAxNzM4OTE5NjEwODEwMzY5.YAlDjg.1Tng_iJDjNLAjI8srTBO0Jed0YM")